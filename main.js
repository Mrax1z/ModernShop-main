...
// --- Simple authentication (client-side demo) ---
const demoUsers = [
  { username: 'admin', password: 'admin123', name: 'Administrator' }
];

function isLoggedIn() {
  return !!localStorage.getItem('shop_user');
}
...
function updateAuthUI() {
  ...
  if (isLoggedIn()) {
    ...
    welcome.textContent = `Hi, ${escapeHtml(user.name || user.username)}`;
...
function setupEventListeners() {
  // Initialize auth UI and login modal handlers
  updateAuthUI();
  const closeLoginBtn = document.getElementById('close-login');
  if (closeLoginBtn) closeLoginBtn.addEventListener('click', (e) => { e.preventDefault(); hideLoginModal(); });
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      if (found) {
        ...
        hideLoginModal();
        alert('Berhasil masuk sebagai ' + found.name);
      } else {
        alert('Username atau password salah. Gunakan akun demo: admin / admin123');
      }
    });
  }

  // Admin access button — arahkan ke halaman login admin
  const adminAccessBtn = document.getElementById('adminAccessBtn');
  if (adminAccessBtn) {
    adminAccessBtn.addEventListener('click', (e) => {
      e.preventDefault();
      // arahkan ke halaman admin-login.html
...
const botResponses = {
    ...
    "ada produk apa saja?": "Kami punya berbagai produk menarik!...",
    "berapa harga termurah?": "Kami punya produk mulai dari Rp 50.000!...",
    // TAMBAHKAN DI SINI ↓
    "stok": "Untuk informasi stok terkini, silakan cek di detail produk atau hubungi WhatsApp kami.",
    "ongkir": "Gratis ongkir untuk pembelian di atas Rp 200.000! Cek halaman produk untuk detail.",
    "garansi": "Semua produk kami bergaransi 1 tahun. Garansi mencakup kerusakan pabrik.",
    "warna": "Produk tersedia dalam berbagai warna. Cek di gambar produk untuk pilihan warna.",
    "ukuran": "Ukuran tersedia dari S sampai XL. Lihat tabel size guide di deskripsi produk.",
    "refund": "Kebijakan refund dalam 7 hari jika produk cacat. Hubungi CS untuk proses lebih lanjut."
};

// Ganti fungsi sendMessage menjadi:
sendMessage.addEventListener('click', function() {
    const message = chatInput.value;
    if (message.trim()) {
        // Encode message untuk URL
        const encodedMessage = encodeURIComponent(`[Live Chat Toko] ${message}`);
        // Ganti NOMOR_ANDA dengan nomor WhatsApp bisnis
        window.open(`https://wa.me/6281234567890?text=${encodedMessage}`, '_blank');
        
        // Tambahkan pesan konfirmasi
        addMessage("Mengarahkan ke WhatsApp...", 'bot');
        setTimeout(() => {
            addMessage("Silakan lanjutkan chat di WhatsApp. Tim kami siap membantu! 📱", 'bot');
        }, 500);
    }
});